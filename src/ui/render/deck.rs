...
use super::{
    centered_rect, ACCENT, BG, BG2, BLUE, CYAN, GOLD, GRAY, GREEN, RED, WARN, WHITE, YELLOW,
};
...
pub(super) fn render_add_to_deck(f: &mut Frame, app: &App, area: Rect) {
    ...
    let title = Paragraph::new(Line::from(vec![
        ...
        Span::styled(
            "  —  select a deck to add words, or create a new one",
...
pub(super) fn render_search_deck(f: &mut Frame, app: &App, area: Rect) {
    ...
    let mut items: Vec<ListItem> = app
        ...
        .map(|(i, w)| {
            ...
            let target_deck = if app.deck_name_input.is_empty() {
                "My Deck"
            } else {
                &app.deck_name_input
            };
            ...
            ListItem::new(Line::from(vec![
                Span::styled(
                    format!(
                        "{}{}  {}  {}  {}",
                        prefix, w.hanzi, w.pinyin, w.english, level_str
                    ),
                    style,
                ),
    ...
    let deck_name = if app.deck_name_input.is_empty() {
        "My Deck"
    } else {
        &app.deck_name_input
    };
    let remove_hint = if !app.deck_name_input.is_empty() {
        "  •  R Remove from deck"
    } else {
        ""
    };
...
pub(super) fn render_add_custom_word(f: &mut Frame, app: &App, area: Rect) {
    ...
    let status_color = if app.dict_status.starts_with("Error") {
        RED
    ...
    if has_filter {
        let filter_text = format!(
            "/{}{}",
            app.dict_filter,
            if app.dict_filter_active { "█" } else { "" }
        );
    ...
    let items: Vec<ListItem> = filtered
        ...
        .map(|(i, e)| {
            ...
            let line1 = Line::from(vec![
                Span::styled(
                    format!("{}{}", prefix, e.hanzi),
                    Style::default()
                        .fg(if selected { CYAN } else { GOLD })
                        .bg(bg)
