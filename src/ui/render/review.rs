...
use super::{ACCENT, BG, BG2, BLUE, CYAN, GOLD, GRAY, GREEN, RED, WHITE, YELLOW, centered_rect};
...
fn render_tone_legend(f: &mut Frame, area: Rect) {
    ...
    let tones = Line::from(vec![
        ...
        Span::styled("   2 ", Style::default().fg(GREEN).add_modifier(Modifier::BOLD)),
        Span::styled("á  rise ", Style::default().fg(GREEN)),
        Span::styled("   3 ", Style::default().fg(YELLOW).add_modifier(Modifier::BOLD)),
        Span::styled("ǎ  dip  ", Style::default().fg(YELLOW)),
        Span::styled("   4 ", Style::default().fg(RED).add_modifier(Modifier::BOLD)),
