...
pub(super) fn render_stats(f: &mut Frame, app: &App, area: Rect) {
    ...
    let kv = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(
                format!("{}/{}", stats.mature_cards, stats.total_cards),
    ...
    let kv_p = Paragraph::new(kv).block(
        Block::default()
            .borders(Borders::ALL)
            .border_type(BorderType::Rounded)
    ...
    // Level breakdown
    let level_items: Vec<ListItem> = stats
        .level_stats
        .iter()
        .map(|ls| {
            let seen_pct = if ls.card_count > 0 {
                ls.reviewed * 100 / ls.card_count
            } else {
                0
            };
            let bar = "█".repeat((seen_pct / 10) as usize);
            let (label, color) = if let Some(deck) = &ls.deck_name {
                let truncated = if deck.len() > 12 {
                    format!("{}…", &deck[..11])
                } else {
                    deck.clone()
                };
            ...
            ListItem::new(Line::from(vec![
                ...
                Span::styled(
                    format!("{:░<10} {:3}% seen", bar, seen_pct),
                    Style::default().fg(GRAY),
                ),
                Span::styled(
                    format!("  {} learned", ls.mature),
                    Style::default().fg(GREEN),
                ),
            ]))
        })
    ...
    let level_list = List::new(level_items).block(
        Block::default()
            .borders(Borders::ALL)
            .border_type(BorderType::Rounded)
            .border_style(Style::default().fg(BLUE))
    ...
    let weak_items: Vec<ListItem> = stats
        .weakest
        .iter()
        .map(|w| {
            let ef_color = if w.ease_factor < 1.8 {
                RED
            } else if w.ease_factor < 2.2 {
                YELLOW
            } else {
                GREEN
            };
            ListItem::new(Line::from(vec![
                ...
                Span::styled(
                    format!("[EF:{:.1}]", w.ease_factor),
                    Style::default().fg(ef_color),
                ),
            ]))
        })
        .collect();

    let weak_list = List::new(weak_items).block(
        Block::default()
            .borders(Borders::ALL)
            .border_type(BorderType::Rounded)
    ...
    let legend_lines = vec![
        ...
        Line::from(Span::styled(
            "  Starts at 2.5, floor 1.3, no ceiling.",
            Style::default().fg(GRAY),
        )),
        Line::from(""),
        Line::from(vec![
    ...
    let legend_p = Paragraph::new(legend_lines).block(
        Block::default()
            .borders(Borders::ALL)
            .border_type(BorderType::Rounded)
    ...
    let max_daily = stats
        ...
        .unwrap_or(1)
        .max(1);
    let daily_lines: Vec<Line> = stats
        .daily_reviews
        .iter()
        .map(|(day, cnt)| {
